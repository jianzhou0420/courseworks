...
for t = 1:851
    ...
    Pseudo_ranges_rate = readmatrix('Pseudo_range_rates.csv');
    time = Pseudo_ranges(t+1,1);
    r_aj_n = zeros(8,1);
    r_dot_aj_n = zeros(8,1);
    H = zeros(16,8);
    r_eb_e = x_n(1:3,:);
    v_eb_e = x_n(4:6,:);
    for i = 1:8
    ...
    r_sd = 10;
    rr_sd = 0.05;
    r_sd = r_sd^2*ones(1,8);
    rr_sd = rr_sd^2*ones(1,8);
    R =diag([r_sd rr_sd]);

    %k) (Step 7) Compute the Kalman gain matrix using
    K = P_n*H.'*inv(H*P_n*H.'+R);
    %l) (Step 8) Formulate the measurement innovation vector
    current_p_r = Pseudo_ranges(t+1,2:9).';
    current_p_rr = Pseudo_ranges_rate(t+1,2:9).';
    clock_offset = x_n(7);
    clock_drift = x_n(8);
    delta_z = [current_p_r-r_aj_n-clock_offset*ones(8,1);
        current_p_rr-r_dot_aj_n-clock_drift*ones(8,1)];
