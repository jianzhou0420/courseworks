...
youbot_dh_parameters = {'a': [0, 0, 0.155, 0.135, 0],
                        'alpha': [0, -np.pi / 2, 0, 0, np.pi / 2],
                        'd': [0.072, 0.075, 0, 0, 0],
                        'theta': [0, 0, -np.pi / 2, 0, np.pi / 2]}

name_link = ['link1', 'link2', 'link3', 'link4', 'link5']
...
def forward_kinematics(dh_dict, joints_readings, up_to_joint=5):
    ...
    for i in range(up_to_joint):
        # T = np.dot(T, standard_dh(dh_dict['a'][i], dh_dict['alpha'][i], dh_dict['d'][i], dh_dict['theta'][i]+joints_readings[i]))
        T = np.dot(T, standard_dh(dh_dict['a'][i], dh_dict['alpha'][i], dh_dict['d'][i],
                              dh_dict['theta'][i]))
    # your code ends here -------------------------------
